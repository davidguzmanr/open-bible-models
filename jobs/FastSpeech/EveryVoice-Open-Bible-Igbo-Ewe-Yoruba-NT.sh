#!/usr/bin/env bash
#SBATCH --job-name=EveryVoice-Open-Bible-Igbo-Ewe-Yoruba-NT
...
#   on preprocess + train, or edit config/everyvoice-text-to-spec.yaml. Re-run preprocess
#   with --overwrite when switching multilingual on/off so artifacts match the model config.
...
everyvoice preprocess config/everyvoice-text-to-spec.yaml \
    --config-args model.multilingual=true \
